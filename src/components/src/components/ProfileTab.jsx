...
import { Loader2, Eye, EyeOff, Save, KeyRound, Trash2 } from "lucide-react";
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card className="p-5 flex flex-col gap-4">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#14263A] to-[#4C7A2E] flex items-center justify-center text-xl font-extrabold text-[#E7EEF4] shrink-0">
            {initialsFrom(form.fullName || user?.email)}
          </div>
          <div className="min-w-0">
            <div className="font-bold text-sm truncate">{form.fullName || "بدون نام"}</div>
            <div className="text-[12px] text-[#7189A0] truncate">{user?.email}</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={saveProfile} className="flex items-center gap-2 bg-[#6FA83E] text-[#0A1622] font-bold px-4 py-2.5 rounded-xl hover:brightness-110">
            <Save size={16} /> ذخیره پروفایل
          </button>
          {savedMsg && <span className="text-[#7FD9A8] text-sm">{savedMsg}</span>}
        </div>
      </Card>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Field label="رمز عبور فعلی">
          <div className="relative">
            ...
            />
            <button type="button" onClick={() => setShowCurrent((s) => !s)} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#7189A0]">
        ...
        <Field label="رمز عبور جدید">
          <div className="relative">
            ...
            />
            <button type="button" onClick={() => setShowNext((s) => !s)} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#7189A0]">
        ...
        <div className="md:col-span-2 flex items-center gap-3">
          <button disabled={busy} className="flex items-center gap-2 bg-[#1B3349] text-[#7FD9A8] font-semibold px-4 py-2.5 rounded-xl hover:brightness-110 disabled:opacity-60">
            {busy && <Loader2 size={16} className="animate-spin" />}
            تغییر رمز عبور
          </button>
          {msg && <span className={`text-sm ${msgTone === "good" ? "text-[#7FD9A8]" : "text-[#E88A7A]"}`}>{msg}</span>}
        </div>
      </form>
    </Card>
  );
...
function DeleteAccountCard({ deleteAccount, logout }) {
  ...
  return (
    <Card className="p-5 flex flex-col gap-3 border-[#5A3128]">
      <div className="flex items-center gap-2 font-bold text-sm text-[#E88A7A]">
        <Trash2 size={16} /> حذف حساب کاربری
      </div>
      <p className="text-[#9DB4C7] text-sm">
        با حذف حساب، تمام اطلاعات مولدها و تنظیمات گله شما برای همیشه پاک می‌شود و این عمل قابل بازگشت نیست.
      </p>
      {!confirming ? (
        <button onClick={() => setConfirming(true)} className="self-start px-4 py-2 rounded-xl text-sm bg-[#3A1F1B] text-[#E88A7A] font-bold">
          حذف حساب کاربری
        </button>
      ) : (
        <div className="flex flex-col gap-3">
          ...
          {msg && <div className="text-[#E88A7A] text-sm">{msg}</div>}
          <div className="flex gap-2">
            <button onClick={() => setConfirming(false)} className="px-4 py-2 rounded-xl text-sm text-[#9DB4C7] hover:bg-[#1B3349]">
              انصراف
            </button>
            <button
              onClick={handleDelete}
              disabled={busy || !password}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm bg-[#5A3128] text-[#E88A7A] font-bold disabled:opacity-60"
            >
              {busy && <Loader2 size={16} className="animate-spin" />}
